...
void deadlock_observer(void) {
	if (getpid() == vaterpid) {
		...
		/* Pruefung ob Deadlock existiert und wenn ja, Aufloesung des Deadlock */
		if((array[0] == 1) & (array[1] == 1)) {
			printf("\033[0;32mBeide Priester warten und streiten sich bald um die zweite Schriftrolle.\nIch  Befehle Bemenophis die Bibliothek zu verlassen.\033[0m\n");

			/* Eliminierung des Priesters Bemenophis und Entfernung des Zombies */
			kill(priester_pids[1], SIGTERM);
			pid_t res;
			res = waitpid(priester_pids[1], NULL, WNOHANG);
			if(res == -1) {
				if(errno != ECHILD) {
					perror("waitpid");
					exit(EXIT_FAILURE);
			...
			/* Setzen des Semaphors des Priesters auf den Ausgangsstatus "Eintreten", also 0. */
			set_sem(semid_priester, 1, 0);
			/* Setzen des Semaphores der Schriftrollen auf 1, da Bemenophis seine Schriftrolle nicht mehr belegt. */
			set_sem(semid_schriftrollen, 0, 1);
			/* Neuerzeugung des Priesters Bemenophis */
			priester_pids[1] = erzeugePriester(1);
			}
